...
**TL;DR table** at the top: one row per finding, `<a href="#c-NNN">` linked, with novelty + confidence pills inline. TL;DR rows are summary layers — regenerate them whenever the finding body changes (see `citation-discipline.md`).

The HTML is a designed document, not a markdown render. Tables, grids, pill systems, two-column blocks are the point. If your HTML reads like a `pandoc` output, restart from the template.
